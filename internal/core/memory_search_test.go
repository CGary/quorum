...
func seedSearchMemory(t *testing.T, db *sql.DB) {
	...
	for _, e := range entries {
		_, err := db.Exec(`INSERT INTO memory_entries (id, project_id, type, source_task, title, context, content, hash)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, e.id, e.projectID, e.typ, "SQL-05", e.title, e.context, e.content, e.id+"-hash")
		if err != nil {
			t.Fatalf("insert memory entry failed: %v", err)
		}
	}
	_, err := db.Exec(`INSERT INTO memory_anti_patterns (entry_id, anti_pattern) VALUES (?, ?), (?, ?)`,
		"PAT-2026-05-31-1", "Do not use embeddings for basic search.",
		"PAT-2026-05-31-1", "Do not search every project by default.")
	if err != nil {
