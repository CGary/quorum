...
func searchMemoryLike(db *sql.DB, query string, opts MemorySearchOptions, limit int) ([]MemorySearchResult, error) {
	...
	where := []string{`(
		...
		LOWER(COALESCE((SELECT group_concat(ap.anti_pattern, ' ') FROM memory_anti_patterns ap WHERE ap.entry_id = e.id), '')) LIKE ?
	)`}
	...
	stmt := `SELECT e.id, COALESCE(e.project_id, ''), e.type, COALESCE(e.title, ''), COALESCE(e.context, ''), e.content,
		COALESCE((SELECT group_concat(ap.anti_pattern, char(10)) FROM memory_anti_patterns ap WHERE ap.entry_id = e.id), '')
