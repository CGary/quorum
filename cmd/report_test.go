...
// legacyReportPayload returns a representative legacy v1.0 report (top-level
// properties, no `content` block). The migrate command consumes this shape and
// emits a semantic v1.1 report. The fields are chosen so the migrated output is
// schema-valid: meta carries a real id/date, the verdict is a string, summary is
// a string, and keyFindings entries supply every field the findings-section
// mapper reads (finding/whyItMatters/action) so no null leaks into the output.
func legacyReportPayload(id string) string {
	return `meta:
  id: "` + id + `"
  schemaVersion: "1.0"
  date: "2026-06-01T00:00:00Z"
verdict: "Legacy verdict bottom line"
summary: "Legacy summary of the report."
decisionSurface:
  "Status": "Investigated"
  "Owner": "team"
keyFindings:
  - finding: "The cache layer is unbounded"
    whyItMatters: "It can exhaust memory under load"
    action: "Add an eviction policy"
`
}

func writeLegacyReport(t *testing.T, dir, id, payload string) string {
	t.Helper()
	reportsDir := filepath.Join(dir, ".ai", "reports")
	if err := os.MkdirAll(reportsDir, 0755); err != nil {
		t.Fatalf("mkdir reports dir: %v", err)
	}
	reportPath := filepath.Join(reportsDir, id+".yaml")
	if err := os.WriteFile(reportPath, []byte(payload), 0644); err != nil {
		t.Fatalf("write legacy report: %v", err)
	}
	return reportPath
}

func TestReportMigrateDryRun(t *testing.T) {
	bin, dir := setupReportTestEnv(t)
	dbPath := filepath.Join(t.TempDir(), "memory.db")

	id := "legacy-dry-01"
	legacy := legacyReportPayload(id)
	reportPath := writeLegacyReport(t, dir, id, legacy)

	out, err := runMemoryCmdErr(t, dir, bin, dbPath, "", "report", "migrate", id, "--dry-run")
	if err != nil {
		t.Fatalf("report migrate --dry-run failed: %v\n%s", err, out)
	}

	// The migrated YAML is printed to stdout in semantic v1.1 shape.
	for _, want := range []string{"kind:", "presentation:", "content:", "sections"} {
		if !strings.Contains(out, want) {
			t.Errorf("dry-run output must contain %q; got:\n%s", want, out)
		}
	}

	// --dry-run must NOT mutate the on-disk file: it stays the legacy original.
	onDisk, err := os.ReadFile(reportPath)
	if err != nil {
		t.Fatalf("read report after dry-run: %v", err)
	}
	if string(onDisk) != legacy {
		t.Errorf("dry-run must not modify the on-disk report.\nwant:\n%s\ngot:\n%s", legacy, onDisk)
	}
}

func TestReportMigrateWritesValidSemanticReport(t *testing.T) {
	bin, dir := setupReportTestEnv(t)
	dbPath := filepath.Join(t.TempDir(), "memory.db")

	id := "legacy-write-01"
	reportPath := writeLegacyReport(t, dir, id, legacyReportPayload(id))

	out, err := runMemoryCmdErr(t, dir, bin, dbPath, "", "report", "migrate", id)
	if err != nil {
		t.Fatalf("report migrate failed: %v\n%s", err, out)
	}

	// The file on disk is now a semantic report.
	migrated, err := os.ReadFile(reportPath)
	if err != nil {
		t.Fatalf("read migrated report: %v", err)
	}
	m := string(migrated)
	for _, want := range []string{"content:", "sections", "schemaVersion"} {
		if !strings.Contains(m, want) {
			t.Errorf("migrated report must contain %q; got:\n%s", want, m)
		}
	}

	// The migrated report must be VALID against report.schema.json (same engine
	// as `report save`). validate --schema report drives it directly.
	if vout, verr := runMemoryCmdErr(t, dir, bin, dbPath, "", "validate", "--schema", "report", reportPath); verr != nil {
		t.Fatalf("migrated report must validate against report.schema.json: %v\n%s\n--- migrated ---\n%s", verr, vout, m)
	}

	// Key-data preservation: legacy verdict string -> content.verdict.text, and a
	// legacy keyFinding -> a findings-role section.
	if !strings.Contains(m, "Legacy verdict bottom line") {
		t.Errorf("legacy verdict must be preserved as content.verdict.text; got:\n%s", m)
	}
	if !strings.Contains(m, "role: findings") {
		t.Errorf("legacy keyFindings must become a findings-role section; got:\n%s", m)
	}
	if !strings.Contains(m, "The cache layer is unbounded") {
		t.Errorf("legacy finding text must be preserved; got:\n%s", m)
	}
}

func TestReportMigrateOnSemanticReportFails(t *testing.T) {
	bin, dir := setupReportTestEnv(t)
	dbPath := filepath.Join(t.TempDir(), "memory.db")

	// An already-semantic report (has top-level `content`) must be refused.
	id := "already-sem-01"
	writeLegacyReport(t, dir, id, validReportPayload(id))

	out, err := runMemoryCmdErr(t, dir, bin, dbPath, "", "report", "migrate", id)
	if err == nil {
		t.Fatalf("migrate on a semantic report must fail, but it succeeded:\n%s", out)
	}
	if !strings.Contains(out, "already semantic") {
		t.Errorf("expected 'already semantic' error, got: %s", out)
	}
}
