...
> **Bounded exception (ADR 0004, 0006, 0007).** A narrow, read-only visualization capability is sanctioned in `quorum serve` (configurable bind host): the `/q-report` skill authors transient report artifacts the viewer renders (ADR 0004); curated SQLite memory is browsable read-only (ADR 0006); and task lifecycle state under `.ai/tasks/{inbox,active,done,failed}` is observable read-only (ADR 0007). This does NOT make Quorum a documentation tool or a human-centric operational UI — the viewer adds no verdict and no merge authority, never mutates task state, worktrees or the memory DB, and never rewrites the append-only `07-trace.json`. Under ADR 0007 the viewer MAY display lifecycle artifacts read-only: `00`/`01`/`04`/`05`/`06` and `feedback.json` in full (collapsed), `02-contract.yaml` and `07-trace.json` summarized only — observation is not a verdict (Rules #4, #6, #8 intact). Operational artifacts remain machine-first. Any further expansion (PDF export, themes, a report builder, state mutation, edit/transition buttons) requires a new ADR. See `docs/adr/0004-visor-reportes-read-only.md`, `docs/adr/0006-visor-memories-read-only.md`, `docs/adr/0007-visor-estado-tareas-read-only.md`.
